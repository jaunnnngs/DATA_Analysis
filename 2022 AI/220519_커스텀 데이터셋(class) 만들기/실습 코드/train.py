...
def main():
    ...
    for epoch_idx in range(epoch):
        ...
        torch_ = torch.save(model.state_dict(), f'{save_root}/{epoch_idx}.pth')

        #정확도가 제일 높은 모델 'best.pth' 이름으로 저장
        max(accuracy).save(model.state_dict(),f'{save_root}/{epoch_idx}.pth')
